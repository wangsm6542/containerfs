...
func Dial(host string) (*grpc.ClientConn, error) {
	...
	var err error
	conn, err = grpc.Dial(host, grpc.WithMaxMsgSize(MaxMsgSize), grpc.WithInsecure(), grpc.WithBlock(), grpc.WithTimeout(time.Millisecond*300), grpc.FailOnNonTempDialError(true))
	if err != nil {
